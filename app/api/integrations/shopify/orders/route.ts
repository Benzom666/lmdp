...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (deliveryOrdersError) {
      ...
      return NextResponse.json(
        { error: "Failed to fetch delivery orders", details: deliveryOrdersError.message },
        { status: 500 },
      )
    }
    ...
    const enhancedOrders = shopifyOrders.map((shopifyOrder) => {
      ...
      if (deliveryOrder) {
        if (deliveryOrder.status === "delivered" && shopifyOrder.fulfillment_status !== "fulfilled") {
          actualFulfillmentStatus = "pending_fulfillment"
          syncStatus = "pending"
      ...
      return {
        ...
        delivery_completed_at: deliveryOrder?.updated_at || null, // Using updated_at directly
